...
describe('cookieParser()', function () {
  ...
  describe('when no cookies are sent', function () {
    ...
    it('should default req.signedCookies to {}', function (_, done) {
      request(createServer('keyboard cat')).get('/signed').expect(200, '{}', done);
    });
  });
...
describe('cookieParser.signedCookie(str, secret)', function () {
  ...
  describe('when secret is an array', function () {
    it('should return false for tampered signed string', function () {
      assert.strictEqual(
        cookieParser.signedCookie('s:foobaz.N5r0C3M8W+IPpzyAJaIddMWbTGfDSO+bfKlZErJ+MeE', [
          'keyboard cat',
          'nyan cat'
        ]),
        false
    ...
    it('should return unsigned value for first secret', function () {
      assert.strictEqual(
        cookieParser.signedCookie('s:foobar.N5r0C3M8W+IPpzyAJaIddMWbTGfDSO+bfKlZErJ+MeE', [
          'keyboard cat',
          'nyan cat'
        ]),
        'foobar'
    ...
    it('should return unsigned value for second secret', function () {
      assert.strictEqual(
        cookieParser.signedCookie('s:foobar.JTCAgiMWsnuZpN3mrYnEUjXlGxmDi4POCBnWbRxse88', [
          'keyboard cat',
          'nyan cat'
        ]),
        'foobar'
...
describe('cookieParser.signedCookies(obj, secret)', function () {
  ...
  it('should include tampered strings as false', function () {
    assert.deepEqual(
      cookieParser.signedCookies({ foo: 's:foobaz.N5r0C3M8W+IPpzyAJaIddMWbTGfDSO+bfKlZErJ+MeE' },
        'keyboard cat'
      ), {
        foo: false
      }
  ...
  it('should include unsigned strings', function () {
    assert.deepEqual(
      cookieParser.signedCookies({ foo: 's:foobar.N5r0C3M8W+IPpzyAJaIddMWbTGfDSO+bfKlZErJ+MeE' },
        'keyboard cat'
      ), {
        foo: 'foobar'
      }
